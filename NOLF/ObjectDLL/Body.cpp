...
*/
        ...
        m_DeathScene.SetNoise(pSurf->fDeathNoiseModifier * (bLand ? 2.0f : 1.0f), _GetTime() + GetRandom(0.5f, 1.0f));
	}
}
...
void Body::Update()
{
	...
	LTBOOL bUpdatingDims = _GetTime() < m_fStartTime + 2.0f;
	...
	if ( m_eBodyState == eBodyStateNormal )
	{
		switch ( m_eBodyStatePrevious )
		{
			...
			case eBodyStateAcid:
				...
				return (_GetTime() > m_fStartTime + 4.0f);
				break;
		}
	}
