...
void CAIBrain::Init(CAIHuman* pAIHuman, const char* szName)
{
	...
	m_eDodgeStatusLast = eDodgeStatusOk;
	m_fDodgeStatusCheckTimeVector = _GetTime() + LOWER_BY_DIFFICULTY(m_pBrain->fDodgeVectorCheckTime);
	m_fDodgeStatusCheckTimeProjectile = _GetTime() + LOWER_BY_DIFFICULTY(m_pBrain->fDodgeProjectileCheckTime);

	m_fGrenadeStatusCheckTime = _GetTime() + LOWER_BY_DIFFICULTY(m_pBrain->fAttackGrenadeThrowTime);
}
...
{
	m_fDodgeStatusCheckTimeVector = _GetTime() + LOWER_BY_DIFFICULTY(m_pBrain->fDodgeVectorDelayTime);
	m_fDodgeStatusCheckTimeProjectile = _GetTime() + LOWER_BY_DIFFICULTY(m_pBrain->fDodgeProjectileDelayTime);
}
...
void CAIBrain::GetDodgeStatus(DodgeStatus* peDodgeStatus, Direction* peDirection, DodgeAction* peDodgeAction, uint32* pdwNode)
{
	...
	if ( _GetTime() >= m_fDodgeStatusCheckTimeVector )
	{
		m_fDodgeStatusCheckTimeVector = _GetTime() + LOWER_BY_DIFFICULTY(m_pBrain->fDodgeVectorCheckTime);
	...
	if ( _GetTime() >= m_fDodgeStatusCheckTimeProjectile )
	{
		m_fDodgeStatusCheckTimeProjectile = _GetTime() + RAISE_BY_DIFFICULTY(m_pBrain->fDodgeProjectileCheckTime);
...
{
	if ( _GetTime() >= m_fGrenadeStatusCheckTime )
	{
		m_fGrenadeStatusCheckTime = _GetTime() + LOWER_BY_DIFFICULTY(m_pBrain->fAttackGrenadeThrowTime);
...
RangeStatus CAIBrain::GetRangeStatus()
{
	...
	if ( _GetTime() > m_fRangeStatusCheckTime )
	{
		// It's time for a fresh report

		m_fRangeStatusCheckTime = _GetTime() + 3.0f;
		bCanReport = LTTRUE;
	}
